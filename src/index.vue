...
const organizationName: Ref<string> = ref('KIS');
const organizationEmail: Ref<string> = ref('kis.ac');
...
function handlePasswordReset() {
  console.log(`Email: ${recovoryEmail.value}`);
  passwordReset.value = false;
  recovoryEmail.value = '';
  openDialog('Check your email!', 'If the email you entered is valid, you will receive an email with a link to reset your password.');
}
    ...
    <div class="model__content p-6 glass-morphic-window w-96 absolute top-1/5 left-1/2 transform -translate-x-1/2">
      ...
      <div class="dialog__body">
        <form class="space-y-6" @submit.prevent="handlePasswordReset">
          ...
          <div>
            <button type="submit" :disabled="errorRecovoryEmail !== 'valid'" class="big-green-button">Send email</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div class="flex min-h-full flex-1 flex-col justify-center px-6 py-12 lg:px-8">
      ...
      <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        <form class="space-y-6" @submit.prevent="handleLogin">
          ...
          <div>
            <button type="submit" @click="openDialog('Sign in', 'You have successfully signed in!')"
              :disabled="(errorEmail !== 'valid') || (password === '')" class="big-green-button">
              Sign in</button>
          </div>
        </form>

        <p class="mt-10 text-center text-sm">
          Not a member? <a href="#" @click="signUp = true">Sign up</a> with your {{ organizationName }} email!
        </p>
      </div>
    </div>
  </div>
